...
func (ks *KubenetesState) gatherJob(j v1batch.Job, acc telegraf.Accumulator) error {
	...
	if j.Status.CompletionTime == nil {
		return nil
	} else if ks.MaxJobAge.Duration < time.Now().Sub(j.Status.CompletionTime.Time) {
	...
	completeTime := j.Status.CompletionTime.Time

	acc.AddFields(jobMeasurement, fields, tags, completeTime)
